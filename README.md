...
> *In short — a daily, orchestrated data pipeline (Steam reviews → immutable raw zone → Spark/Iceberg silver → dbt/DuckDB gold) feeding a sentiment model tracked in MLflow, served by a FastAPI endpoint and a Streamlit dashboard, with blocking data-quality gates, drift monitoring, rollback tooling, 149 automated tests and 28 mutation tests run in CI.*
...
| Preuve | Résultat | Où |
|---|---|---|
| Batterie de tests | **149 verts**, en intégration continue | onglet [Actions](https://github.com/KinSushi/reviewpulse-pipeline/actions) |
| **Tests inverses** : un défaut volontaire est injecté (pseudonymisation supprimée, convention de décision inversée, barrière de promotion retirée…) et un test **nommé** doit le détecter | **28 mutations sur 28 tuées**, en un seul passage, sur un runner GitHub | [`tests/reverse/mutations.json`](tests/reverse/mutations.json) |
...
Le jour d'une démonstration : `sh tools/jour_j.sh` ([notice](docs/presentation/JOUR_J.md)) lève la pile, attend qu'elle soit chaude et vérifie qu'elle est cohérente. Sur une pile déjà levée : `make prevol` — la pile est-elle levée, **réchauffée** et cohérente ? Le premier appel après un redémarrage charge le modèle ; le contrôle le fait avant le jury.
...
tests/               149 tests ; tests/reverse/ : les 28 mutations
