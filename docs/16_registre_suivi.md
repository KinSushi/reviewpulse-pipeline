...
| R44 | Le chiffre « 26 mutations sur 26 » du dernier `KNOWN_GOOD` n'a aucun journal sur disque | audit du 19/09 | **P1** | ⬜ | Un journal de campagne daté, conservé dans `docs/evidence/`, montrant 125 tests et 26 mutations tuées | Seuls deux journaux existent, tous deux antérieurs aux correctifs de M17 et M22 ; le plus récent rapporte **24 tuées, 2 survivantes** | Relancer `make campagne` avec le journal écrit dans `docs/evidence/`, corriger l'entrée KG-2026-09-19-g, **et vérifier la minute 4:40 de `script_10_minutes.md`** qui annonce ces deux chiffres au jury |
