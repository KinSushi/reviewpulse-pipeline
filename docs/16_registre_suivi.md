...
| R04 | Six présentations calibrées (Demo Day 10 min ; CDSD 7-8 diapositives ; AIA 1 15 min ; AIA 2, 3 et 4 5 min) | référentiels | P1 | 🔄 | Un jeu par soutenance, avec script parlé, minuté | **2 sur 6** : `ReviewPulse_DemoDay.pptx` (9 diapositives) et `ReviewPulse_AIA4.pptx` (5). Le constructeur est piloté par une **spécification JSON** et refuse désormais un indice de run inexistant, témoin à l'appui ; le deck du Demo Day est reproduit **au bit près** après refonte | Chiffres à rafraîchir après les preuves, puis CDSD, AIA 1, AIA 2, AIA 3 |
