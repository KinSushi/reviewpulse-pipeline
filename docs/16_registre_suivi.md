...
| R44 | Le « 26 sur 26 » est une composition de deux passages, pas une campagne unique | audit du 19/09 | P2 | 🔄 | Un seul journal, de bout en bout, dans `docs/evidence/`, montrant 125 tests et 26 mutations tuées | Les journaux perdus ont été retrouvés **dans les conteneurs arrêtés** et versés dans `docs/evidence/` : `batterie_125_tests.log` (125 passed en 8 min 07) et `mutations_M17_M22.log` (deux TUEE). Le chiffre est donc établi, mais en deux temps : 24 à la campagne de 21 h 17, 2 au passage ciblé qui a suivi | Relancer `make campagne` de bout en bout quand la machine sera libre ; la minute 4:40 de `script_10_minutes.md` doit dire « en deux passages » tant que c'est le cas |
