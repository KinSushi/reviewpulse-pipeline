...
*Qualité* – Contrôles bloquants avant écriture de la zone propre (ADR 0005). Great Expectations couvre **les trois zones** : la zone propre (`expectations.py`, qui lit `config.CLEAN_FILE`) et, depuis le 19/09/2026, les zones silver et gold (`expectations_lake.py`, ADR 0020). Mesuré en exécution réelle du DAG le 20/09/2026 : quatre suites, **29 attentes évaluées, zéro échec** — `silver_reviews` 8, `silver_predictions` 7, `gold_faits` 7, `gold_mart` 7 (`docs/evidence/dag_execution_reelle.md`). Tests dbt sur la zone gold : le compte déclaré dans les contrats est de **30** (5 `unique`, 17 `not_null`, 3 `relationships`, 5 `accepted_values`) ; le chiffre de 43 qui circulait n'avait aucune trace exécutée sur disque. Contrats dbt dans `_staging.yml` et `_marts.yml`, tous deux présents sur le disque.  
...
**b. Sur le pilote ReviewPulse**  
Nettoyage : suppression des champs identifiants dans la zone propre. Validation : contrôles bloquants (ADR 0005), Great Expectations, 30 tests dbt déclarés dans les contrats. Normalisation : transformation en zone propre, pseudonymisation stable. Surveillance : PSI sur le flux naturel, alerte écrite dans un fichier daté hors du journal Airflow, déclenchement de réentraînement. Aucun outil de profilage ou tableau de bord dédié à la qualité n’est mentionné. Aucun indicateur de qualité agrégé n’est présenté. Aucun programme de formation n’est décrit.
