...
| Données de la zone **silver** | Contenu des tables Iceberg (ex. `silver.reviews`) | Instantanés Iceberg | Consultation via `lakehouse.table_history("<namespace>.<table>")` | Aucun outil de restauration d’instantané n’est encore fourni ; seules la lecture et la liste sont disponibles. |
...
- Implémentation d’une commande de restauration d’un instantané Iceberg (ex. `lakehouse.restore_snapshot`).  
