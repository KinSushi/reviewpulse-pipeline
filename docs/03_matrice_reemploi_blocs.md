...
| `fraud-mlops-control-tower` | **clone local** : `D:/SAS/reference/sources-archive/external_sources/_KINSUSHI/fraud-mlops-control-tower` — et GitHub public | propre | synthétiques | Seuil choisi par F1, fiche modèle, fiche données → gabarits des documents **AIA 4** ; logique métier de la fraude → **AIA 3**. Le clone porte six documents dont ReviewPulse n'a pas l'équivalent : `docs/deployment_runbook.md`, `docs/risk_assessment.md`, `docs/data_card.md`, `docs/monitoring_plan.md`, `docs/public_safety.md`, `docs/local_run_report.md`. **Retrouvé le 20/09/2026** par une recherche disque lancée avant de découvrir que Julie était accessible ; il était noté « non cloné » depuis le 16/09. À lire comme modèle de format, jamais à recopier dans un livrable |
