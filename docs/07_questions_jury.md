...
Parce qu'un test classique prouve qu'un chemin autorisé passe, jamais qu'un contrôle refuse. Une batterie entièrement verte peut coexister avec une porte de qualité qui laisse tout passer : aucun test ne l'aurait jamais sollicitée. Une mutation injecte un défaut volontaire dans une copie du code — la convention de décision inversée, la pseudonymisation supprimée, la barrière de promotion retirée — puis relance la batterie et exige qu'un test **nommé** échoue. Si aucun n'échoue, la mutation « survit », et c'est un trou de couverture réel, jamais un test à assouplir. Nous en avons vingt-huit, chacune reliée au test qui la tue. Le 20/09/2026, sur un runner d'intégration continue vierge, 27 mutations sur 27 ont été tuées en un seul passage, zéro survivante ; la vingt-huitième, ajoutée le 21/09, inverse la part négative prédite du résumé quotidien — le chiffre que l'utilisateur lit chaque matin, qu'aucun test ne vérifiait en valeur. Les deux survivantes de la veille avaient chacune révélé un défaut : l'une, une porte non prouvée bloquante ; l'autre, un test qui passait pour la mauvaise raison. → ADR 0005.
...
**Vous avez fait documenter et journaliser le code par des modèles : comment savez-vous qu'ils n'ont rien cassé ?**
Parce qu'aucun fichier réécrit n'entre dans le dépôt sans franchir cinq portes mécaniques, et que nous avons compté ce qu'elles refusent. Le rendu doit être du Python valide ; il ne doit avoir ni perdu ni **inventé** une citation d'ADR ou de test ; il ne doit pas expliquer moins que la version qu'il remplace ; son arbre syntaxique, docstrings et appels de journal retirés, doit être **identique** à celui de l'original ; et l'original est conservé. Les dix-neuf modules de production sont passés ainsi, et les refus sont la partie instructive : dans `score.py`, la ligne `scored["decision_threshold"] = threshold` **supprimée** sans un mot — la porte de qualité de la zone scorée aurait échoué au passage suivant du DAG ; ailleurs une variable ajoutée, un `print` de tableau converti en journal invalide, huit « Pourquoi » sur huit effacés par un modèle chargé de les corriger. Trois familles de modèles se partagent les rôles — auteur, relecteur, arbitre — et la relecture automatique s'est trompée plus d'une fois sur deux : ce qui tranche, ce sont les contrôles d'arbre, eux-mêmes éprouvés par dix-huit témoins en intégration continue. Une porte, pas une confiance. → `docs/23_standard_agents.md`.
