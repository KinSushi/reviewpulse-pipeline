...
**Vous avez fait enrichir le code par un modèle : comment savez-vous qu'il n'a rien cassé ?**
Parce qu'aucun fichier réécrit n'entre dans le dépôt sans franchir quatre portes mécaniques, et que nous avons compté ce qu'elles refusent. Le rendu doit être du Python valide ; il ne doit avoir perdu aucune citation d'ADR, de test ou de registre ; son arbre syntaxique, docstrings et appels de journal retirés, doit être **identique** à celui de l'original ; et l'original est copié en quarantaine avant d'être remplacé. Le 20/09/2026, sur **neuf rendus** d'un modèle local, quatre ont été appliqués et **cinq refusés** : une syntaxe cassée, une variable intermédiaire ajoutée, deux docstrings ayant perdu leurs tests associés — et, dans `score.py`, la ligne `scored["decision_threshold"] = threshold` **supprimée** sans un mot. La porte de qualité de la zone scorée, qui contrôle cette colonne, aurait échoué au passage suivant du DAG. Aucune relecture ne l'aurait vu dans onze mille caractères ; la comparaison d'arbres l'a nommé en une seconde. Une porte, pas une confiance. → `docs/23_standard_agents.md`.
