...
| S1-6 | DAG : ingest → spark_silver → gx → score → **drift** → **gold** ; retries ; alertes d'échec et de SLA. **Exécuté le 19/09 : 6 tâches sur 6.** Airflow migré sur PostgreSQL et `LocalExecutor` : le scheduler survit désormais aux interrogations concurrentes | AIA 3 C3.3, ind. 6.1 | P1 | ✅ |
